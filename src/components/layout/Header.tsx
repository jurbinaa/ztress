...
export const Header: React.FC = () => {
  ...
  return (
    <header className="w-full flex justify-between items-center z-10 glass-panel px-6 py-4 organic-rounded-2xl border border-white/[0.08] organic-shadow-lg organic-transition-slow">

      {/* ── Identidad de marca ─────────────────────────────────────────── */}
      <div className="flex items-center gap-3">
        {/* Círculo con icono de spa — logotipo de la aplicación */}
        <div className="size-10 organic-rounded bg-gradient-to-br from-primary/80 to-primary/20 flex items-center justify-center shadow-[0_0_15px_rgba(var(--theme-primary-rgb),0.3)] organic-breath">
          <span className="material-symbols-outlined text-on-primary text-[24px]">
            spa
          </span>
        </div>
        <div>
          <h1 className="font-display text-xl tracking-wide font-medium text-on-surface organic-text-display">Ztress</h1>
          <p className="font-body text-[10px] uppercase tracking-widest text-on-surface-variant/70 font-semibold organic-text">Santuario Mental</p>
        </div>
      </div>

      {/* ── Selector de tema de color ──────────────────────────────────── */}
      <div className="flex items-center gap-1.5 bg-background/50 p-1.5 organic-rounded border border-white/5">
        {THEMES.map((theme) => (
          <button
            type="button"
            key={theme.id}
            onClick={() => setThemeColor(theme.id)}
            className={`size-8 organic-rounded flex items-center justify-center organic-transition ${
              themeColor === theme.id
